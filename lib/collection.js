...
Collection.prototype.update = function (search, update, opts, fn) {
  ...
  this.col.update(search, update, opts, function(err, result) {
    callback(err, opts.multi ? result && result.result.nModified : null)
  });
...
Collection.prototype.remove = function (search, opts, fn) {
  ...
  this.col.remove(search, opts, function(err, result){
    promise.resolve(err, result && result.result.n);
  });
...
Collection.prototype.aggregate = function (stages, opts, fn) {
  ...
  opts = this.opts(opts);
  var promise = new Promise(this, 'aggregate', opts);

  if (fn) {
    promise.complete(fn);
  }

  promise.query = stages;
  this.col.aggregate(stages, opts, promise.resolve);
